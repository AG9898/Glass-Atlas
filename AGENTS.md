...
### 2026-07-07 — Chat retrieval only uses notes with current semantic indexes; refresh via script, not SQL
`CHAT-07B` centralized semantic-index freshness in `src/lib/server/db/notes.ts`: status must be `current`, note-level embedding and index timestamps must exist, and `semantic_index_source_updated_at >= updated_at`. Chat semantic and lexical retrieval now filter to that contract, so stale/pending/failed published notes disappear from answers until maintenance runs. Use `npm run semantic-index:audit` to inspect published-note readiness and `npm run semantic-index:refresh` to reindex non-current notes through `reindexNoteAfterSave()`; never patch vectors or `note_chunks` directly in SQL.

### 2026-07-07 — `npm run chat:audit` is the scripted live chat sweep; empty 200 SSE streams are a real, frequent nemotron failure mode

### 2026-07-14 — Removed the Neon keepalive; it was burning Techy's shared CU-hr quota, and `InitialLoadScreen` never masked cold-start latency anyway
`src/lib/server/db/keepalive.ts` (added 2026-05-20) pinged `SELECT 1` every 4 minutes to avoid Neon compute cold-starts. Techy and Glass Atlas share one Neon project/branch (Techy owns `public`, Glass Atlas owns `glass_atlas` — see `drizzle.config.ts`), so this ping kept the *shared* compute permanently active and was the direct cause of Techy's Neon account approaching its monthly 100 CU-hr cap (~82.58/100) while showing no real traffic — confirmed via `neonctl operations list`, which showed the endpoint stuck in `ready` with zero `suspend_compute` events for 7+ straight days, versus a sibling Neon project on the same account autosuspending normally in 5–10 minute cycles. Removed the import from `hooks.server.ts` and deleted the file; do not re-add a periodic DB ping without first considering whether it should live on a Techy-isolated Neon project instead of the shared one.

Note for whoever revisits this: `InitialLoadScreen.svelte`'s `MIN_DISPLAY_MS` floor does **not** compensate for the cold-start latency this ping was masking. That component only mounts in `onMount` — i.e. after the browser has already received the SSR'd HTML. The homepage's `+page.server.ts` (`listNotes`, `getTotalCitations`) and other DB-touching `load()` functions are plain blocking `await`s, so a cold Neon compute delays *time-to-first-byte* — the phase before `InitialLoadScreen` exists in the DOM at all. A visitor hitting a cold compute sees a blank/native browser loading state for the cold-start duration, then gets the normal SSR'd page (with its now-irrelevant-to-this 1500ms splash) on arrival. If cold-start-masking is wanted back, it needs to happen upstream of first paint — e.g. streamed/deferred `load()` data so the shell (and `InitialLoadScreen`) ships immediately while the DB query resolves in the background — not a change to the splash's timing floor.

### 2026-07-14 — Homepage `load()` now streams `landing` data instead of blocking, so the shell reaches the browser before a cold Neon compute responds
Following the keepalive removal above, `src/routes/+page.server.ts`'s `load()` returns `{ landing: loadLandingData() }` **unawaited** — SvelteKit sends the initial HTML shell (nav, `InitialLoadScreen`, hero) immediately and streams the resolved `{ stats, latestNotes }` in afterward via a trailing `<script>` chunk (`Transfer-Encoding: chunked`). `+page.svelte` wraps the stats grid and latest-notes section in `{#await data.landing}...{:then landing}...{:catch}`, with a pulsing `.stat-skeleton` placeholder and a "Loading the latest notes…" line for the pending state (respects `prefers-reduced-motion`). The hero's GSAP entrance timeline was split out from the stats/latest timeline since hero content doesn't depend on `data.landing` and shouldn't wait on it; the stats/latest timeline now runs from a `$effect` gated on a `landingResolved` flag (set when `data.landing` resolves) instead of firing blindly from `onMount`, mirroring the existing `/notes` page's shell+motionKey `$effect` pattern rather than introducing a new one. Verified via the CDP technique from `docs/TESTING.md`: initial response body contains `stat-skeleton` markup and omits `data-motion="stat"` (proving the `{:then}` branch isn't in the synchronous SSR output), and a follow-up `Runtime.evaluate` after load confirmed real stat values, zero leftover skeleton nodes, and a normally-faded splash.

**Deliberately not applied to `/notes` or `/notes/[slug]`.** `/notes`' `load()` output (`data.notes`) is deeply wired into client-side filter reactivity (`notesMotionKey` derived from `data.notes.map(...)`, re-fetched via `goto()` on every filter change) — turning that into a streamed promise would require reworking the filter reactivity chain, which is out of scope for a cold-start fix. `/notes/[slug]` calls `error(404, ...)` inside `load()` when a note is missing/unpublished, which requires knowing the result before the response commits — incompatible with streaming that specific check. Both pages are near-always reached by clicking through from `/` or `/notes` within the same session (client-side navigation over an already-woken compute), so the realistic cold-start scenario — a visitor's very first hit — is covered by the homepage fix. Revisit only if analytics ever show meaningful direct/cold traffic landing straight on `/notes/[slug]`.
`CHAT-07E` added `scripts/chat-audit.js` (manual tooling, never CI — it burns free-model quota and bumps citation counters): retrieval diagnostics via Vite SSR imports, live `POST /api/chat` SSE capture with a fresh anonymous session per request (so the per-session quota never trips), quote-copy shared-word-run diagnostics, and direct OpenRouter probes that distinguish HTTP errors from in-band SSE `error` events. The sweep confirmed tiers/fallback/sources all behave correctly, but `nvidia/nemotron-3-ultra-550b-a55b:free` returned HTTP 200 + a single in-band `502 ResourceExhausted` error event + zero content on 3 of 7 citable questions — the adapter treats 200 as success so the fallback never engages and the client shows an empty bubble with a Sources control (`CHAT-08A`). A tiny probe prompt succeeding does not clear a model: the failures only showed up under real RAG-sized prompts.

### 2026-09-18 — Production canonical origin is `https://glassatlas.dev`
Railway serves both the apex and `www`, but `canonicalHostRedirect` must stay the first request-handling hook in `hooks.server.ts` so `www` permanently redirects to the apex before Auth.js handles the request; only `securityHeaders` sits ahead of it, wrapping the chain so the 308 still carries HSTS. `buildCanonicalRedirectUrl()` assigns `pathname`/`search` onto the canonical origin rather than resolving the request path relative to it — `new URL('//evil.com/x', origin)` resolves to `https://evil.com/x`, an open redirect off the brand domain. Keep the Railway-provided domain attached and exempt from that redirect as a rollback/diagnostic path; `PUBLIC_SITE_URL` is the apex and `AUTH_URL` stays unset per RESOLVED-20.

### 2026-09-18 — Collection-level chat questions need a metadata lane; retrieval cannot reach them at all
"What is the most recent note?" / "how many notes have you written?" returned the no-coverage fallback not because of a tuning problem but because publication order and corpus size live in **table columns, not chunk prose** — semantic search embeds such a query to noise, lexical search matches no title/tag/category, and the confidence gate fires. Reading more notes into context cannot fix this; the ordering fact is not in the text being read. `POST /api/chat` now has a catalog lane (`detectCatalogIntent` + `buildCatalogContext` in `chat.ts`, `listPublishedNoteCatalog()` in `db/notes.ts`) between the social lane and retrieval. Three rules worth keeping: (1) the intent patterns must stay a conservative allowlist requiring a corpus noun — a false negative just falls through to RAG, a false positive answers a content question with a table of contents, so every new pattern needs a paired negative test; (2) the lane answers through the LLM, with the deterministic response as an outage-only safety net, because a title-plus-link reply hands the reader off instead of telling them what the note says; (3) it deliberately skips the `CHAT-07B` semantic-freshness filter, since that filter guards stale *chunk vectors* and this lane reads live columns — filtering it would make the chat misreport the published corpus mid-reindex. See RESOLVED-27.

### 2026-09-18 — The chat cannot say what model it runs on, and that is a routing gap, not a policy
`detectSocialIntent`'s `identity` case matches only the substrings `who are you` / `what are you`, so "what model are you and what provider are you" misses every lane and falls through to RAG, which has no note documenting the site's own stack — the visitor gets "I don't have a note on that yet," which reads as evasive rather than accurate. The real answer (OpenRouter; `nvidia/nemotron-3-ultra-550b-a55b:free` primary, `openrouter/free` fallback) lives only in `openrouter.ts`; `/how-it-works` names the provider but deliberately no model slug. Left unfixed pending an author decision on how much to disclose. If it is ever wired up, name the provider and link `/how-it-works` rather than hardcoding a model slug — the free-model default has already been retired upstream three times (`gemini-2.0-flash-001`, `gemma-4-31b-it:free`, and the `gpt-oss-*:free` family), so a slug in prose will drift.

### 2026-09-18 — Styleguide tokens are not live until they exist in every `app.css` theme block
`docs/styleguide.md` and the handoff token file already defined the warm-taupe `--color-accent2-*` scale, but `src/app.css` omitted it, so SVG declarations such as `stroke: var(--color-accent2-700)` computed to `none` and silently hid the semantic graph layer. When adopting a documented token, verify it exists in the base `:root`, system-dark media block, and class-based `:root.dark`; an undefined custom property can fail without a build or lint error.
