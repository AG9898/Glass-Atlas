...
Railway serves both the apex and `www`, but `canonicalHostRedirect` must stay first in `hooks.server.ts` so `www` permanently redirects to the apex before Auth.js handles the request. Keep the Railway-provided domain attached and exempt from that redirect as a rollback/diagnostic path; `PUBLIC_SITE_URL` is the apex and `AUTH_URL` stays unset per RESOLVED-20.
