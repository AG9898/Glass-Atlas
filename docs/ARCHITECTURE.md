...
- Chat component manages optimistic UI state ("searching notes…"), token streaming, and citation link rendering today. The backend source-metadata contract (`POST /api/chat`'s trailing `sources` SSE event) shipped in CHAT-06A; the planned subtle source control — a button on assistant messages that opens a popup with note titles and brief snippets, each title linking to its note — is CHAT-06B and not yet implemented in `Chat.svelte`.
...
14. API route calls `buildChatSources(citedNotes)`; when it returns at least one source, the route
    ...
    snippet }, ...] }` is appended right after the LLM stream completes (shipped: CHAT-06A). The
    planned frontend source-popup UI that consumes this event is CHAT-06B and not yet built — once
    shipped, it would show a subtle source button on assistant messages with this metadata, listing
    retrieved note titles and brief snippets; clicking a source title would navigate to `/notes/[slug]`.
```
...
**Source transparency (backend contract shipped, CHAT-06A):** Public chat source metadata is driven by retrieval metadata, not by LLM-generated prose. `buildChatSources()` in `chat.ts` derives `{ slug, title, snippet }` entries from `citedNotes` — the same candidates already selected for prompt assembly — and drops unsafe slugs or empty snippets using `isSafeNoteSlug()`. `POST /api/chat` only calls it on the citable LLM path (high/borderline confidence) and only emits the trailing `sources` SSE event when at least one source remains; the low-confidence fallback and social-intent lane never call it. Snippets are collapsed-whitespace, truncated, HTML-escaped excerpts from the cited chunk text or takeaway (`buildSourceSnippet()` in `chat-format.ts`) — never full note bodies. The frontend popup UI that renders this metadata (CHAT-06B) is still pending; coverage labels, if shown there, should stay subtle and not distract from reading the answer.
