...
- Render public pages: landing (`/`), notes list (`/notes`), note detail (`/notes/[slug]`), global node view (`/node-view`), and how-it-works (`/how-it-works`). There is no standalone chat route — chat lives in the landing hero; the former `/chat` redirect stub and the nav's `CHAT` anchor link were removed in favor of a `HOME` nav link. The landing `+page.server.ts` loads live homepage metrics (published note count, distinct topic count, average words per published note, total citations served) and up to 7 most recent published notes via `db/notes.ts` helpers; it remains chat-first and uses automatic note-derived content.
- `/how-it-works` (`src/routes/how-it-works/+page.svelte`, shipped `POLISH-05`) is a static informational page — no `+page.server.ts`, no DB or LLM calls. It explains, in order: what a note is, how the chat's grounded retrieval works (framed narratively — no privacy/rate-limit/quota specifics), the stack (SvelteKit/Svelte 5, Tailwind + Bits UI, Neon + pgvector via Drizzle, OpenRouter, GitHub OAuth admin, Railway hosting) in a blueprint-recipe technical panel, and a 3-step "how the pieces fit" architecture summary in a second blueprint panel. `Nav.svelte`'s left nav link set is `HOME` (`/`), `NOTES`, `NODE VIEW` (`/node-view`), and `HOW IT WORKS` (`/how-it-works`).
- `/node-view` loads a compact, published-only global graph. Every published note is a node, including notes with no edges. Authored edges resolve `note_links` only when both endpoints are published; semantic edges are calculated server-side from current note-level embeddings, deduplicated, thresholded, and capped per note. Raw embedding vectors and semantic-index errors never cross the server boundary. The full graph distinguishes and independently toggles authored versus semantic edges, supports drag/pan/zoom, and opens notes on activation.
...
- `db/notes.ts` — note CRUD helpers: `listNotes(filter?)`, `getNoteBySlug`, `createNote`, `updateNote`, `deleteNote`, `getBacklinks`, `getOutlinks`; graph helper: `getPublishedNoteGraph()` (returns compact published-note nodes plus separately typed authored and semantic edges, never bodies or vectors); RAG helpers: `searchNotesBySimilarity` (published notes ordered by pgvector cosine distance), `replaceNoteChunks`, `searchChunksBySimilarity` (chunk-level cosine similarity over published notes with current semantic indexes only), `searchNotesByLexical` (natural-language topic/token matching against title/tags/category, filtered to published notes with current semantic indexes), `recordCitations`, `getTotalCitations`; semantic maintenance helpers: `hasCurrentSemanticIndex()` (central freshness predicate) and `getNoteChunkCount()`; chat quota helper: `consumeChatRateLimit` (atomic window-reset + increment keyed by `chat_rate_limits.session_hash`); reader-path helper: `getRelatedNotes(slug, limit)` (looks up the source note's own current embedding, then returns published/current notes ordered by pgvector cosine distance to it, excluding the source note — empty when the source has no current semantic index; kept separate from `getBacklinks`/`getOutlinks`). Exports plain-object types `Note`, `CreateNoteInput`, `UpdateNoteInput`, `RetrievedLexicalNote`, `RelatedNote`, including note metadata (`image`, `mediaType`, `publishedAt`, `series`) and semantic index state.
- `chat.ts` — `assembleContext()` runs semantic and lexical/topic retrieval in parallel, fuses candidates (semantic-first, lexical support/fill, capped at 5 notes), and assembles a compact context block per note; never injects full note bodies. Context labels frame snippets as evidence to paraphrase/synthesize, not prose to copy: assistant answers should paraphrase and connect retrieved chunks conversationally unless the user asks for a quote. Each `citedNotes` entry carries a `snippet` (HTML-escaped, truncated excerpt from the cited chunk/takeaway via `buildSourceSnippet()` in `chat-format.ts`) alongside `slug`/`title`. `buildChatSources()` filters `citedNotes` to safe-slug, non-empty-snippet entries — this is the source-of-truth contract for public chat source transparency: titles, slugs, and brief retrieved snippets can be exposed to the client, but model-invented citations cannot. Low-confidence fallback ignores retrieved nearest-neighbor candidates entirely so irrelevant notes are not presented as sources or related links.
...
- `personality.ts` — exports the system prompt personality block as a string constant; never inlined elsewhere. The prompt keeps strict grounding while allowing fuller, more conversational answers when the retrieved notes support them.
- `ai/openrouter.ts` — sends chat completion requests to OpenRouter using the configured primary free model and can fall back to the configured free-router model when the primary model is rate-limited, temporarily unavailable, or times out at the HTTP/connect level. The app consumes visible `delta.content` tokens only; reasoning fields from fallback providers are ignored. Beyond connect-level failures, the adapter also probes the primary model's SSE body itself before handing a stream back to the caller: an in-band `{ "error": ... }` event arriving on an HTTP 200 stream, a stream that completes with zero visible content, or a stream that goes quiet for longer than `DEFAULT_OPENROUTER_STREAM_STALL_TIMEOUT_MS` (15s) all trigger the same single fallback-model retry, since nothing has been forwarded to the client yet. Once at least one visible content chunk has been forwarded, a later in-band error or stall instead just closes the stream gracefully with `[DONE]` (no retry, since a retry would duplicate/garble an answer already in flight) and logs the provider detail. If both the primary and fallback attempts fail, the adapter logs the combined provider detail and throws — `POST /api/chat` catches this and returns a `503` JSON error, never an empty `200` SSE stream with a sources event.
...
- `scripts/semantic-index.js` — local maintenance CLI for published-note chat readiness. `npm run semantic-index:audit` reports whether every published note has a current semantic index and at least one chunk row; `npm run semantic-index:refresh` reindexes non-current notes through `reindexNoteAfterSave()` and then reports the post-refresh state. It never writes vectors or chunks directly.
...
- `POST /api/chat` — public; accepts `{ message }` (max 2 000 characters, rejected with 400 otherwise); reads/sets an anonymous `chat_session` cookie, enforces 10 messages per hour per anonymous browser session, serves the author-curated showcase reply when the message is an exact case-insensitive match for the suggested prompt (`SUGGESTED_CHAT_PROMPT` in `chat-format.ts`, "How does this site work?" — a static, factually accurate description of the stack and grounding contract, same trust level as the /how-it-works page; added because the notes corpus does not document the site's own build, so the real RAG path answers this thinly), serves an allowlisted social-intent reply for lightweight small-talk turns (greeting/thanks/capability/identity prompts), serves a **catalog answer** for allowlisted questions about the collection itself (most recent note, how many notes, what topics exist) by querying note metadata directly instead of running retrieval, otherwise embeds query, runs hybrid retrieval, and streams either an index-grounded steer or a normal grounded LLM answer via SSE. On the citable LLM path (retrieval confidence high or borderline), the route calls `buildChatSources(citedNotes)` and, only when it returns at least one source, appends a single trailing SSE `data:` event shaped `{ "sources": [{ slug, title, snippet }, ...] }` after the LLM stream completes. This payload omits the OpenAI `choices` shape so existing/older clients parse it as an empty, ignorable token. The social-intent lane and the low-confidence fallback never call `buildChatSources` and never emit a `sources` event; low-confidence copy should be explicit about missing documentation rather than surfacing unrelated source links.
- `GET /api/note-graph` — public, read-only compact graph payload used by the lazy note-detail expansion dialog. It delegates to `getPublishedNoteGraph()` and returns the same node/edge contract as `/node-view`; no bodies, draft metadata, raw embeddings, or index errors are exposed.
...
- `Nav.svelte` — global navigation shell rendered in `+layout.svelte` on every page. Receives `session` prop from `+layout.server.ts` (loaded via `event.locals.auth()`). Dark mode toggle reads `localStorage('ga-theme')` on mount (falls back to `prefers-color-scheme`), persists the preference, and applies/removes a `.dark` class on `<html>`. The switch itself (`POLISH-07I`) plays a directional wave wipe via the native View Transitions API (`document.startViewTransition`, gated on feature support + `canUseSpatialMotion()`) — CSS in `src/app.css` (`::view-transition-old(root)`/`::view-transition-new(root)`) sweeps the new theme in left-to-right instead of the API's default cross-fade; falls back to an instant swap when unsupported or reduced motion is preferred. Login routes to the custom sign-in page (`/signin`, configured via Auth.js `pages.signIn`) and logout uses Auth.js sign-out action (`/auth/signout`). Search icon links to `/notes?focus=search`, which opens the notes index and auto-focuses the search field.
- Chat component manages optimistic UI state ("searching notes…"), token streaming, citation link rendering, and source-popup rendering. Its empty state offers a one-click suggested-prompt chip ("Try it — How does this site work?") that submits `SUGGESTED_CHAT_PROMPT` through the normal send path, giving first-time visitors an instant demo whose reply is served by the curated showcase lane below. The backend source-metadata contract (`POST /api/chat`'s trailing `sources` SSE event) shipped in CHAT-06A; `Chat.svelte` consumes it via `parseChatSourcesEvent()` (`src/lib/utils/chat-format.ts`) and attaches the result to the in-flight assistant message (CHAT-06B). Each assistant message with sources shows a compact `Dialog`-based trigger (`src/lib/components/ui/Dialog.svelte`) that opens a popup listing each source's title — linking to `/notes/[slug]` — and brief snippet; messages without sources render no control. The panel header also carries an expand toggle (top right): expanding animates the panel from its in-flow hero position into a larger centered fixed-position rectangle (not full screen) over a scrim backdrop, via a GSAP FLIP-style tween. Because the panel normally lives inside the ScrollSmoother-transformed `#smooth-content` wrapper (where `position: fixed` resolves against the transform, not the viewport), the section is reparented to `document.body` for the expanded state and returned to a same-size placeholder in the hero grid on collapse; Escape or a backdrop click collapses it, background scroll is locked while expanded, reduced-motion users get an instant snap with no tween, and any navigation (e.g. clicking a note or How It Works link inside the chat) force-collapses it instantly via `beforeNavigate` plus an unmount-teardown fallback — required because the reparented section sits outside the component subtree and would otherwise survive page teardown as an unclosable stranded overlay.
...
- `NoteGraph.svelte` — mini D3 force graph rendered in the left sidebar of `NoteDetail.svelte`. Receives pre-built `NoteGraphData` (nodes + edges) from the server load; D3 is loaded via a dynamic `import('d3')` inside `$effect` so the module never runs during SSR. Shows the current note's 1-hop wiki-link neighborhood (outlinks + backlinks, published notes only, no forward refs). It stays a small, fixed-zoom supporting widget with node drag/background pan and an expand action that lazily loads the global graph into a centered dialog focused on the current note.
- `FullNoteGraph.svelte` — reusable global D3 graph used by `/node-view` and the note-detail expansion dialog. It receives compact nodes and typed `wiki`/`semantic` edges, exposes independent relationship toggles, and supports node drag, viewport pan/zoom, focus highlighting, click/keyboard navigation, deterministic reduced-motion layout, and full teardown of simulation/zoom listeners.
- `WaveGridLoader.svelte` (`POLISH-07F`, reworked `POLISH-07H`) — the branded motion asset, now a reusable loading/spinner primitive rather than a homepage decoration. Takes a `variant: 'overlay' | 'compact'` prop selecting a grid density/camera config (`getWaveGridConfig()` in `src/lib/motion/wave-grid.ts`). Always renders a static CSS grid-pattern fallback first (SSR-safe, zero-JS, tokenized to `--color-bg`/`--color-line-1`), then, only in the browser and only when `shouldUseWaveGridMotion()` allows it, dynamically imports `three` inside `onMount` and mounts a `<canvas>` overlay driving a sparse `THREE.LineSegments` wireframe grid with a per-vertex sine-wave displacement animated via `requestAnimationFrame`, built by the shared scene/layer builders in `src/lib/motion/wave-grid-engine.ts`. Reduced-motion users never trigger the `three` import at all. Fully torn down (renderer disposed, geometry/material disposed, animation frame cancelled, `ResizeObserver` disconnected) on unmount. `aria-hidden="true"` and `pointer-events: none` throughout. It is mounted in three places: `PageTransitionOverlay.svelte` (`variant="overlay"`, mounted only while the overlay covers the screen, unmounted once the reveal tween completes), `InitialLoadScreen.svelte` (`variant="overlay"`, boot-only splash — see below), and `Chat.svelte` (`variant="compact"`, next to the "Searching notes…" placeholder message while awaiting the first streamed token). It no longer appears on the landing page — the hero-to-stats structural band it previously occupied was removed.
- `InitialLoadScreen.svelte` (`POLISH-07H`) — fullscreen boot splash wrapping `WaveGridLoader` (`variant="overlay"`), mounted in `+layout.svelte` gated to public routes only (same `isPublicSmoothScrollPath` rule as smooth scroll and route transitions — admin stays low-motion). Shows once per hard page load (never on SPA navigation, since the root layout does not remount between client-side routes), fades out via CSS opacity transition once both `document.fonts.ready` has resolved and a fixed 1500ms minimum display time has elapsed (`Promise.all`, no upper cap — a slower load simply holds the splash longer instead of cutting it off) so the splash always reads as a deliberate, persisted moment rather than a flash a fast load blinks past. Skipped entirely when reduced motion is preferred — nothing is rendered, since the page is already server-rendered and usable without it. `pointer-events: none` throughout.
...
4. API route checks a narrow social-intent allowlist (greeting/thanks/capability/identity/how-it-works):
   ...
   - if not matched: continue to step 4b
4b. API route checks a narrow catalog-intent allowlist (most recent note / how many notes / what topics):
   - if matched: queries published-note metadata via `listPublishedNoteCatalog()` (slug, title,
     takeaway, category, publication date — never body or embedding), builds a note-index prompt
     block, and streams an LLM answer grounded in that block; on LLM failure it serves a
     deterministic factual answer instead of a 503. Retrieval and the confidence gate are skipped;
     no citations are recorded
   - if not matched (or nothing is published): continue to step 5
...
6. API route runs retrieval — both branches execute in parallel:
   a. Semantic: chunk-level cosine similarity search via `searchChunksBySimilarity` (top 20 candidates), filtered to published notes whose semantic index is current and non-stale
      chunks grouped by note slug (≤2 chunks per note)
   b. Lexical/topic: bounded natural-language topic tokens extracted from the query, matched via title/tags/category ILIKE search in `searchNotesByLexical` (top 10 candidates), also filtered to published notes with current semantic indexes
...
7. API route applies confidence gating from semantic cosine-distance tiers:
   ...
   - low confidence or empty retrieval: streams an index-grounded steer. The prompt carries a
     metadata index of published notes (title, slug, category, tags, takeaway, date — never
     bodies or chunk prose) plus a steer-only instruction, so the model may name the closest
     notes and describe what ground they cover but may not answer the asked question from that
     metadata; when nothing is plausibly adjacent it says so plainly rather than reciting the
     corpus. No source/related-note links and no `sources` SSE event are emitted on this path.
     An LLM call *is* made here (unlike earlier revisions), so the guarantee is no longer
     structural: it rests on the index carrying no evidence to fabricate from, plus a
     deterministic index-derived response served if the call fails, errors, or returns empty
...
8. API route builds the LLM prompt:
     ...
     + [retrieved note evidence to paraphrase/synthesize — chunks with section headings, lexical-only notes with takeaway]
9. API route calls OpenRouter with the primary free chat model (`nvidia/nemotron-3-ultra-550b-a55b:free` by default, overrideable via `OPENROUTER_MODEL`) and `stream: true`; the adapter retries once through `OPENROUTER_FALLBACK_MODEL` (`openrouter/free` by default) for rate limits, temporary provider failures, timeouts, in-band SSE `error` events, zero-visible-content stream completions, and stalled streams (no data for 15s)
10. OpenRouter streams tokens → the adapter forwards only visible `delta.content` SSE chunks (reasoning fields are ignored) → API route pipes them as SSE to the frontend. If both the primary and fallback models fail, the route returns a `503` JSON error instead of an empty SSE stream.
...
13. Frontend renders italicized related-note links in the assistant output when the citable LLM path uses notes
...
**Current retrieval (CHAT-04D through CHAT-07D shipped):** `POST /api/chat` now has a short social-intent lane before retrieval for lightweight conversational turns, followed by a catalog lane for questions about the note collection itself. Non-social requests continue through `assembleContext()`, which expands narrow local aliases for semantic embedding, then runs semantic chunk retrieval and lexical/topic note retrieval in parallel. Both retrieval branches are filtered to published notes whose semantic index is `current`, has note-level embedding metadata, has an indexed/source timestamp, and is not older than the saved note content. Semantic chunks are grouped by note (≤2 chunks per note) and ranked by cosine distance. Lexical/topic retrieval reduces the original query to a bounded set of meaningful lowercase terms, dropping stop words, duplicates, and punctuation before matching title, category, and tags; this lets full questions provide support for terms such as RAG, embeddings, security, documentation, agents, and loop prompts without turning the entire question into a noisy substring. Lexical-only notes not already in the semantic set are appended in publication-date order. The combined slate is capped at 5 distinct notes. Semantic notes contribute title, section headings, and chunk evidence labeled for paraphrase; lexical-only notes contribute title and takeaway evidence labeled for synthesis. Full note bodies are never sent to the LLM. Confidence gating uses semantic distance tiers exposed on assembled context metadata: high-confidence evidence proceeds to the normal LLM answer path, borderline evidence proceeds to a stricter limited-coverage prompt that must identify the evidence as adjacent or partial, and low-confidence retrieval streams an index-grounded steer without nearest-neighbor related/source links. The steer's index is built from the same `listPublishedNoteCatalog()` helper the catalog lane uses, so there is one corpus-listing query in the codebase, and it is scoped to the low tier only — the high and borderline paths are unchanged and never receive it. Lexical/topic matches are supporting evidence; they do not override a clearly irrelevant semantic distance.
...
- The low-confidence index steer may use note metadata only as navigation surface. It must name and characterize notes, never answer the asked question from a takeaway; it must stop plainly when nothing is plausibly adjacent instead of touring the corpus; it must not emit a `sources` event (source metadata continues to mean "notes this answer was grounded in", and links on this path are inline `[[slug|Title]]` wiki-links rendered by `chat-format.ts`); and it must not be extended to the high or borderline paths without a new decision, since those paths currently work and carry real evidence.
- The catalog lane answers questions *about the collection* (publication order, corpus size, topic coverage) that retrieval is structurally unable to reach, because those facts live in table columns rather than in chunk prose. It reads only note metadata — title, slug, category, takeaway, publication date — so the "never send full note bodies to the LLM" rule holds unchanged. Unlike chat retrieval, it is deliberately **not** filtered by semantic-index freshness: that filter exists to keep stale chunk vectors out of grounded answers, and this lane uses no chunk text, so excluding a mid-reindex note would make the chat report the published corpus incorrectly. It must stay an allowlist — a phrasing that could plausibly be a content question belongs on the retrieval path.
- The curated showcase lane (suggested-prompt chip) is the one sanctioned factual template: its reply is author-written static text about the site's own architecture, matched only by exact prompt equality. Do not add fuzzy matching or LLM-generated content to it, and do not fold it into the social-intent lane.
