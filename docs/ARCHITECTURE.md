...
- `WaveGridField.svelte` (`POLISH-07F`) — the first branded motion asset: a wave-like, line-based 3D grid field rendered as a bounded structural band between the landing page hero and stats sections (`src/routes/+page.svelte`). Always renders a static inline SVG line-grid fallback first (SSR-safe, zero-JS, tokenized to `--color-line-2`/`--color-line-3`/`--color-accent-500`/`--color-accent2-500`), then, only in the browser and only when `shouldUseWaveGridMotion()` (`src/lib/motion/wave-grid.ts`) allows it, dynamically imports `three` inside `onMount` and mounts a `<canvas>` overlay driving a sparse `THREE.LineSegments` wireframe grid with a per-vertex sine-wave displacement animated via `requestAnimationFrame`. Reduced-motion users never trigger the `three` import at all — the static SVG frame is the permanent state. Fully torn down (renderer disposed, geometry/material disposed, animation frame cancelled, `ResizeObserver` disconnected) on unmount. `aria-hidden="true"` and `pointer-events: none` throughout — purely decorative, never intercepts scroll/click/focus.
