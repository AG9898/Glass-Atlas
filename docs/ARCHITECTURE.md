...
5. Form action regenerates semantic indexes in fail-soft mode. The planned ADMIN-11A contract is:
   - Note-level: calls `embedText(body)` and stores the result in `notes.embedding` only when the new embedding is generated successfully
   - Chunk-level: splits body into section/paragraph chunks, builds metadata-enriched embedding payloads (`title`, `category`, `tags`, `series` + chunk text), embeds each chunk, then upserts the full chunk set via `replaceNoteChunks()` only after every chunk embedding is ready
6. If embedding calls fail or return malformed data, the action logs the error, preserves the previous embeddings, and still completes the note save. Admin surfaces should expose stale-index state when saved note content is newer than the stored semantic index.
