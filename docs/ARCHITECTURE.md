...
> **Implementation gotcha — SvelteKit aliases in a plain-Node script.** `createNote()`/`reindexNoteAfterSave()` and their imports use SvelteKit virtual modules and aliases (`$env/dynamic/private`, `$lib/*`) that plain `node` cannot resolve. `scripts/create-note.js` therefore boots Vite in SSR middleware mode, loads local `.env` values into `process.env`, then imports the existing server helpers through Vite's resolver. Reusing those helpers (not re-implementing insert, embedding, chunk, or link logic) is the contract.
