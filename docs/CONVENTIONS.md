...
1) quota check, 2) curated showcase lane (exact case-insensitive match on `SUGGESTED_CHAT_PROMPT` from `src/lib/utils/chat-format.ts` — the one sanctioned author-written factual template; never fuzzy-matched, never LLM-generated), 3) allowlisted social-intent handling (templated/non-factual only), 4) allowlisted catalog lane (metadata questions about the collection itself), 5) retrieval + confidence gate, 6) LLM stream. Do not route factual questions through the social-intent path.
...
**Chat index-steer contract (low-confidence path, server side)** — When retrieval fails the confidence gate, the low-confidence path streams an LLM-composed *steer* grounded in a metadata index of the published notes rather than a fixed string. Build that index from `listPublishedNoteCatalog()` in `src/lib/server/db/notes.ts` — the same helper the catalog lane uses; do not add a second corpus-listing query. The index carries title, slug, category, tags, takeaway, and publication date, never note bodies or chunk prose. The steer instruction belongs in a route-level constant alongside `LIMITED_COVERAGE_INSTRUCTION` in `src/routes/api/chat/+server.ts`, **not** in `SYSTEM_PROMPT`, because it governs one path rather than the assistant's persona. Three rules the instruction must enforce: the model may name notes and say what ground they cover but may never answer the asked question from a takeaway; it must say so plainly and stop when nothing in the index is plausibly adjacent, rather than reciting the corpus at an off-topic question; and it links notes with inline `[[slug|Title]]` wiki-links, which `chat-format.ts` already renders. This path emits **no** `sources` SSE event — that contract continues to mean "notes this answer was grounded in", and an index entry is not evidence. A deterministic index-derived response covers LLM failure, empty output, or an in-band provider error, since the path must not become less reliable than the fixed string it replaced. Scaling ceiling: the index costs roughly 63 tokens per note, so a whole-corpus block stops being appropriate somewhere around 150–200 notes and would need a title-only or pre-filtered variant.

**Chat catalog lane contract (server side)** — Questions about the *collection* of notes rather than their contents (most recent note, how many notes, what topics are covered) are answered by `detectCatalogIntent()` + `buildCatalogContext()` in `src/lib/server/chat.ts`, reading `listPublishedNoteCatalog()` from `src/lib/server/db/notes.ts`. Keep the intent patterns a conservative allowlist: every pattern requires an explicit corpus noun (note/post/article/writing) or an anchored bare phrasing, so a topical question that merely contains a word like "recent" stays on the retrieval path. A false negative is cheap (the query falls through to normal RAG); a false positive answers a content question with a table of contents. The prompt block carries metadata only — title, slug, category, takeaway, publication date — never note bodies or chunk prose, and it is not filtered by semantic-index freshness (see ARCHITECTURE.md). The lane answers through the LLM so the reply explains the notes conversationally rather than dumping titles; `buildCatalogFallbackResponse()` is a deterministic safety net used only when the LLM call fails, because the point of the lane is that this question class becomes reliably answerable. The lane emits source metadata but records no citations — enumerating an index is not the same as citing evidence.
...
**Chat source popup UI (client side, shipped CHAT-06B)** — `Chat.svelte` defines a local `ChatMessage.sources?: ChatSource[]` field (`ChatSource` type lives in `src/lib/utils/chat-format.ts`, not imported from `$lib/server/**`). While reading the SSE stream, every parsed `data:` line is checked with `parseChatSourcesEvent()` before falling back to `extractToken()`; a non-null result calls `setLastAssistantSources()` to attach the array to the in-flight assistant message without touching its streamed text. The template renders a compact `Dialog` (`src/lib/components/ui/Dialog.svelte`) trigger only when `message.sources` is non-empty — no control renders for messages without sources, and the popup lists each source's title (linking to `/notes/[slug]`) plus its snippet. `source.title` is raw and rendered via normal Svelte text interpolation (auto-escaped); `source.snippet` already arrives HTML-escaped from `buildSourceSnippet()` on the server, so it is rendered via `{@html source.snippet}` — do not re-wrap it in `escapeHtml()` or it will double-escape. `parseChatSourcesEvent()` re-validates every entry against `isSafeNoteSlug` plus non-empty title/snippet, even though the server already filters with the same rules, because the wire payload is untrusted from the client's perspective.
...
**Canonical host redirect** — `canonicalHostRedirect` must remain the first request-handling handle in `hooks.server.ts` (only `securityHeaders`, which just decorates the outgoing response, wraps it). It builds the target URL by assigning `pathname`/`search` onto the canonical origin — never by resolving the request path as a relative URL, which lets a protocol-relative path such as `//evil.com/x` escape to another origin. It redirects only `www.glassatlas.dev` to `https://glassatlas.dev` with status `308`, preserving the path and query. Do not redirect localhost or the Railway-provided domain; the latter is the production rollback/diagnostic path.
...
**Three.js in Svelte components (branded motion assets)** — same "never import at module scope" rule as D3: use a dynamic `import('three')` call inside `onMount`, never a static top-level import, since it touches `WebGLRenderer`/canvas APIs that do not exist during SSR. Gate the import behind the shared reduced-motion helper (`shouldUseWaveGridMotion()` / `canUseSpatialMotion()` from `src/lib/motion/`) — reduced-motion users must never trigger the `three` import at all, not just receive a paused/hidden canvas. Render an SSR-safe static fallback (inline SVG or CSS, tokenized to the same palette) unconditionally first, and only mount the `<canvas>` overlay on top once the dynamic import resolves; fail-soft on any init error by staying on the static fallback. Read brand colors from the live CSS custom properties via `getComputedStyle(document.documentElement)` (with hardcoded light-mode hex as a last-resort non-browser fallback) so light/dark parity holds automatically instead of hardcoding theme-specific hex into the scene. Tear down fully on unmount: cancel the `requestAnimationFrame` loop, disconnect any `ResizeObserver`, and dispose every `BufferGeometry`/`Material`/`WebGLRenderer` instance — guard the async setup with a `cancelled` flag exactly like the D3 pattern below so a dynamic import that resolves after unmount never touches a stale canvas. See `src/lib/components/WaveGridLoader.svelte`, `src/lib/motion/wave-grid.ts`, and `src/lib/motion/wave-grid-engine.ts` (`POLISH-07F`, reworked into a reusable loader in `POLISH-07H`) for the reference implementation. The scene/layer construction and geometry math are factored into the engine module (framework-agnostic) so the Svelte component can mount multiple independent instances at once (route-transition overlay, initial-load splash, inline chat spinner), each with its own lifecycle.

**Reader paths** — note detail pages should present semantic related notes plus explicit backlinks/outlinks as separate reader-path concepts. Semantic related notes come from `getRelatedNotes(slug, limit)` in `src/lib/server/db/notes.ts`, which orders published notes by pgvector cosine distance to the source note's own embedding and excludes the source note; public pages must only link to published notes. Backlinks/outlinks must come from the `note_links` table through query helpers, not from scanning full note bodies in route components. Keep the D3 graph small, supporting, and chrome-free: direct manipulation is limited to dragging nodes and panning the fixed-zoom viewport, with no workspace frame, toolbar, minimap, or graph-editor controls. A node drag must suppress click navigation; an ordinary click or keyboard activation on a non-current node still opens `/notes/[slug]`. `NoteGraph.svelte`'s hover interaction dims non-neighboring nodes/links and highlights the hovered node's direct connections via `classed('is-dimmed'/'is-focused'/'is-active', ...)`, with the actual visual transitions (`r`, `opacity`, `stroke-opacity`, `stroke-width`) defined in CSS (`transition: r 0.18s ease, opacity 0.18s ease`) rather than driven from JS, so they compose with the dynamic-import/cleanup lifecycle above. For `prefers-reduced-motion: reduce`, skip the animated force-simulation ticking entirely: call `simulation.stop()`, synchronously advance `simulation.tick()` enough times to converge (`Math.log(alphaMin) / Math.log(1 - alphaDecay)`), and paint the final node/link positions once — never attach the `.on('tick', ...)` listener in that branch. Direct dragging and panning remain usable in reduced-motion mode, but must update positions synchronously without restarting the simulation; CSS hover transitions remain disabled there.

**Global note graph** — `/node-view` and the expanded note-detail pane must consume the same compact `PublishedNoteGraph` contract and render through the same full-graph component. The server graph helper is published-only: include isolated published notes, drop unresolved/draft-authored link endpoints, compute semantic edges only from current note-level embeddings, deduplicate unordered pairs, cap semantic neighbors per note, and apply a named maximum-distance threshold. Never serialize note bodies, raw vectors, semantic-index errors, or draft metadata into the graph payload. Preserve edge provenance as `type: 'wiki' | 'semantic'`; the client must render and toggle those layers separately. The full graph may add wheel/pinch zoom to the mini graph's drag/pan behavior, but it must retain drag-click suppression, keyboard note activation, dynamic D3 import/cleanup, and synchronous reduced-motion convergence. The note-detail dialog fetches `/api/note-graph` lazily and focuses its originating slug; the standalone route loads the same helper server-side.
...
- For animation:
  ...
  - Initialize GSAP through `$lib/motion` only in client lifecycle (`onMount`) and clean up with `createPublicGsapContext(...).then((cleanup) => cleanup())`.
  ...
  - Use `canUseSpatialMotion()` / the `canUseSpatialMotion` context field as the default gate for movement, pinning, scrubbed timelines, parallax, and smooth-scroll behavior.
  - Use `schedulePublicScrollTriggerRefresh()` instead of calling `ScrollTrigger.refresh()` directly from components.
...
- Give the paraphrase instruction a concrete, checkable ceiling rather than a purely qualitative rule (`personality.ts`: "never carry over more than about 8 consecutive words from any excerpt"), set below the `chat:audit` quote-copy flag threshold (12 shared words) with margin. Reinforce the same instruction inline in `chat.ts`'s per-chunk context label (`Evidence to paraphrase (rewrite in your own words, do not copy wording):`) so the reminder sits next to the excerpt text itself, not only in the system prompt. This applies to every retrieved excerpt regardless of confidence tier — a stronger match is not a license to quote it more closely.
...
**Mermaid diagram rendering (server-side, `PUBLIC-05A`)** — ` ```mermaid ` fences render to inline SVG at markdown-render time via `renderMermaidToSvg()` in `src/lib/server/mermaid-render.ts`, called from a dedicated `rehypeRenderMermaidDiagrams` rehype plugin in `src/lib/server/markdown.ts`. That plugin runs before `rehypeRaw`/`rehypeShiki` in the unified pipeline: on success it replaces the `<pre><code class="language-mermaid">` node in place with `<div class="mermaid-diagram">` wrapping the rendered SVG (spliced into the hast tree by `rehype-raw`); on failure (invalid syntax, empty source, or any renderer error) it falls back to the exact same `unhighlighted-code-source` treatment used for plaintext/text/txt fences (shared via `markCodeNodeAsUnhighlighted()`) instead of throwing — a bad diagram never 500s a note page.
- `renderMermaidToSvg()` never throws; it always resolves `{ ok: true, svg } | { ok: false, error }`. It runs the real `mermaid` package (no mocking/approximating diagram syntax) against a single reusable `jsdom` window, with `SVGElement.prototype.getBBox`/`getComputedTextLength`/`getScreenCTM` patched (jsdom implements none of these) so mermaid's D3-based layout can complete. Layout is therefore approximate, not pixel-perfect — an accepted tradeoff to avoid a headless-browser dependency (Playwright/Puppeteer) for occasional note diagrams.
- Because mermaid reads `window`/`document`/etc. as ambient globals (there is no dependency-injection seam for a custom document), `mermaid-render.ts` installs those onto `globalThis` immediately before each render and restores the prior values immediately after, in a `try/finally`, and serializes all renders through a single in-process mutex. **Never** leave these globals installed outside of an in-flight render — doing so would make any other server code that branches on `typeof window` (e.g. GSAP) incorrectly think it is running in a browser for the rest of the process lifetime. Do not change this module to cache/leave the jsdom globals installed as a process-wide singleton.
- Mermaid renders under `securityLevel: 'strict'`, which sanitizes label content internally; this is why the rendered SVG can be spliced into the hast tree as a `raw` node processed by `rehype-raw` even though `remarkRehype` itself is configured with `allowDangerousHtml: false` for markdown-sourced HTML.

**Code block controls (server-side, `PUBLIC-05B`)** — every remaining `<pre><code>` block (Shiki-highlighted, plain-text fallback, or failed-Mermaid fallback) is wrapped in a `.ga-code-block` blueprint panel by `rehypeWrapCodeBlocks` in `src/lib/server/markdown.ts`, which runs last in the rehype chain (after `rehypeShiki`/`rehypeFixDarkThemeForeground`, before `rehypeStringify`) so it wraps the final `<pre>` markup unchanged. The wrapper markup is `div.ga-code-block > (div.ga-code-block__header > div.ga-code-block__meta + div.ga-code-block__controls) + div.ga-code-block__body > pre`. A language label (`span.ga-code-block__lang`, uppercased) renders only when a language is known (from a `language-*` class or a `data-language`/`dataLanguage` property); a fence with no info string renders no language label. Fence info strings may carry an optional `filename="..."` attribute after the language (for example ` ```ts filename="app.ts" `); `rehypeExtractCodeMeta` reads this from the mdast `code` node's `data.meta` (set by `mdast-util-to-hast`'s default code handler) immediately after `remarkRehype` runs, before `rehypeRaw` — `.data` does not survive `rehypeRaw`'s tree-wide stringify+reparse pass, so this step cannot move later. Both controls are static markup only: `button[data-role="copy"]` and `button[data-role="wrap-toggle"][aria-pressed]`; actual click behavior is wired up client-side.
- **Property-naming gotcha:** `rehypeRaw` (via `hast-util-raw`) round-trips the *entire* hast tree through HTML (stringify then reparse) even when no literal raw node is present, and `hast-util-from-parse5`/`property-information` normalize any `data-*` HTML attribute into a camelCase hast property name (`data-filename` → `dataFilename`) during that reparse. A property set with a hyphenated key **before** `rehypeRaw` runs will silently end up camelCased when read later in the pipeline; a property set with a hyphenated key **after** `rehypeRaw` stays hyphenated. `rehypeExtractCodeMeta` therefore sets `dataFilename` (camelCase) directly since it runs pre-`rehypeRaw`. `markCodeNodeAsUnhighlighted()` sets `data-language` and is called from both sides of `rehypeRaw` (pre-raw for the failed-Mermaid fallback inside `rehypeRenderMermaidDiagrams`, post-raw for the plaintext-fence fallback inside `rehypeUnsupportedCodeAsPlainText`), so `extractCodeLanguage()` in `rehypeWrapCodeBlocks` checks both `'data-language'` and `dataLanguage` property keys to cover either case. Any future rehype step that reads a property set earlier in this pipeline must account for this normalization if it runs after `rehypeRaw`.
- Client-side wiring (`src/lib/components/NoteDetail.svelte`) is a single delegated `click` listener bound to the `.note-body` container ref (`bind:this={bodyEl}`) inside a `$effect`, since the `.ga-code-block` controls live inside a static HTML string injected via `{@html bodyHtml}` and are not individually bound Svelte elements. Wrap-toggle flips the `.ga-code-block--wrap` modifier class on the closest `.ga-code-block` and mirrors state via `aria-pressed`; copy reads `.ga-code-block__body code`'s `textContent` (stripping the mdast code handler's trailing `\n`) and calls `navigator.clipboard.writeText()`, flashing a `.ga-code-block__control--copied` class for 1.5s on success and failing silently (no user-facing error) if the Clipboard API is unavailable or rejects. The shared `.ga-code-block*` classes live in `src/app.css` under `@layer components` (never duplicated in route-scoped `<style>` blocks) since the injected HTML needs them to resolve globally, following the same pattern as `ga-select-*`. Both control states (wrap-active, copy-feedback) change only `border-color`/`color`/`background`, never element size, so toggling never shifts surrounding layout.
...
- There is no DB keep-alive ping. The Neon compute is shared with the Techy project (same project/branch, separate `glass_atlas` schema) and a 4-minute `SELECT 1` ping previously kept that shared compute from ever autosuspending, burning Techy's CU-hr quota for both apps. Do not reintroduce a periodic DB ping — see the 2026-07-14 discovery entry in `AGENTS.md` before adding one.
...
- Plain-text fenced blocks (`plaintext`/`text`/`txt`) are accepted in notes but rendered as unhighlighted code blocks before `rehype-shiki` runs (the legacy Shiki stack cannot tokenize them). Mermaid fences are rendered to real SVG diagrams via `renderMermaidToSvg()` (see Public Markdown Technical Blocks above) — do not revert them to the plaintext/unhighlighted-code path except as the fail-soft fallback for a render failure.
