...
**Reader paths** — note detail pages should present semantic related notes plus explicit backlinks/outlinks as separate reader-path concepts. Semantic related notes come from `getRelatedNotes(slug, limit)` in `src/lib/server/db/notes.ts`, which orders published notes by pgvector cosine distance to the source note's own embedding and excludes the source note; public pages must only link to published notes. Backlinks/outlinks must come from the `note_links` table through query helpers, not from scanning full note bodies in route components. Keep the D3 graph small and supporting; polish should improve fluidity and interaction clarity without turning it into the primary reading surface.
