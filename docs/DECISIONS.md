...
### RESOLVED-28 — Corpus Index Steer on the Low-Confidence Chat Path

**Resolved:** 2026-09-18
**Decision:** When retrieval fails the confidence gate, show the model a metadata index of the published notes (title, slug, category, tags, takeaway, date) and have it compose a steer, replacing the fixed `"I don't have a note on that yet."` string. Four scoping choices: (1) the index is added on the **low tier only** — high and borderline paths are untouched; (2) the model may **name and characterize notes but never answer the asked question from a takeaway**, and must say so plainly and stop when nothing is plausibly adjacent rather than reciting the corpus; (3) notes are linked with inline `[[slug|Title]]` wiki-links and the path emits **no** `sources` SSE event; (4) the LLM composes the steer, with a deterministic index-derived response served on failure, empty output, or an in-band provider error.
**Why:** The fixed string is identical whether the corpus is genuinely silent on a topic or whether embedding search simply missed a note that plainly covers it, so a reader gets a dead end and no way to discover what exists. At the current corpus size the full index is ~580 tokens, cheap enough to show the model the whole table of contents and let it pick the most useful redirect. This supersedes the "Follow-on (not implemented)" note on RESOLVED-27.
**What is being traded away:** RESOLVED-21 and the earlier confidence-gate work made this path structurally safe — no LLM call meant fabrication was impossible. That property is deliberately given up. The replacement guarantee is narrower and rests on two things rather than on structure: the index carries no evidence to fabricate from (metadata only, no bodies or chunk prose), and the prompt forbids answering from it. Any future change that puts evidence-like text into this block reopens this decision.
**Alternatives rejected:** An always-on index in every prompt was rejected because it changes grounding conditions on paths that currently work, to fix a problem only the low tier has. Extending it to the borderline path was rejected for the same reason. Allowing a hedged one-line answer from a takeaway was rejected because a summary used to answer a question is evidence regardless of hedging, and it would overstate coverage. Two-step re-retrieval (model picks a slug, server fetches that note's chunks, model answers grounded) was rejected for now as the most useful but most expensive option: it adds an LLM round-trip to the slowest path on a provider documented as intermittently failing (RESOLVED-25, `CHAT-08A`) — it remains the natural upgrade if the steer proves valuable. A `sources` popup on this path was rejected because it would redefine source metadata from "notes this answer was grounded in" to "notes that were nearby".
**Affects:** `src/lib/server/chat.ts`, `src/routes/api/chat/+server.ts`, `docs/PRD.md`, `docs/ARCHITECTURE.md`, `docs/CONVENTIONS.md`, `docs/TESTING.md`
**Assumptions recorded:** The index is not filtered by semantic-index freshness, consistent with the catalog lane — so the steer can point at a note that a follow-up question cannot yet answer about, if that note is mid-reindex. The index includes category and tags, since topical surface is what makes a steer accurate, at roughly 15% more block size.

### RESOLVED-27 — Chat Catalog Lane for Collection-Level Questions

**Resolved:** 2026-09-18
**Decision:** Answer questions about the note *collection* (most recent note, how many notes, what topics are covered) through a dedicated metadata lane in `POST /api/chat`, placed after the social-intent lane and before retrieval. The lane matches a conservative phrase allowlist (`detectCatalogIntent`), reads published-note metadata via `listPublishedNoteCatalog()` — slug, title, takeaway, category, publication date, never body or embedding — and passes that index block to the LLM with a per-intent instruction so the reply explains the notes conversationally instead of printing titles. `buildCatalogFallbackResponse()` serves a deterministic factual answer if the LLM call fails. The lane emits source metadata but records no citations, and is not filtered by semantic-index freshness.
**Why:** These questions were structurally unanswerable, not merely poorly retrieved. Publication order and corpus size live in table columns, not in chunk prose, so semantic search embeds them to noise and lexical search finds no title/tag/category match; both branches miss and the confidence gate returns "I don't have a note on that yet" for a fact one indexed query away. Verified live before the fix: "What is the most recent note that the author wrote about?" returned the no-coverage fallback. No amount of additional reading or context would fix this, because the ordering fact is not in the text being read.
**Alternatives rejected:** A deterministic template as the primary response was rejected because it answers with a title and hands the reader off rather than telling them what the note says; it is kept only as an outage safety net. Giving the model a `search_notes` tool and letting it loop was rejected on three grounds: it adds round-trips to a path that is already the worst experience, it breaks the "always stream, never buffer" contract (no token can ship until the tool loop resolves), and free-model tool-calling reliability is worse than plain completion on a provider already documented as intermittently failing. Filtering the lane by semantic-index freshness (as chat retrieval does per RESOLVED-25/`CHAT-07B`) was rejected because that filter exists to keep stale *chunk vectors* out of grounded answers; this lane reads live table columns and carries no chunk prose, so applying it would make the chat misreport what is published during a reindex.
**Affects:** `src/lib/server/chat.ts`, `src/lib/server/db/notes.ts`, `src/routes/api/chat/+server.ts`, `docs/ARCHITECTURE.md`, `docs/CONVENTIONS.md`, `docs/TESTING.md`
**Follow-on (not implemented):** An always-on corpus index block in every prompt — the same metadata, carried on the normal retrieval path so that ordinary retrieval misses can still name adjacent notes. At the current corpus size the full index is ~580 tokens, so it is affordable, but it changes grounding behavior on every request (the model could synthesize from a one-line takeaway and overstate coverage) and therefore needs its own task and its own decision.
...
**`CHAT-08A` fix (2026-07-07):** `src/lib/server/ai/openrouter.ts` no longer treats an HTTP 200 response as automatically successful. Before returning a stream to the caller, it now reads the primary model's SSE body itself: an in-band `{ "error": ... }` event, a stream that completes with zero visible `delta.content`, or a stream that stalls past a new 15s inactivity timeout (`DEFAULT_OPENROUTER_STREAM_STALL_TIMEOUT_MS`) all trigger the existing single fallback-model retry, exactly like the pre-existing HTTP-level retry conditions (rate limit/provider failure/timeout). Once at least one visible content chunk has been forwarded to the client, the same failure modes instead close the stream gracefully with `[DONE]` and log the detail — a retry at that point would duplicate or garble an answer already in flight. If both models fail, the adapter logs combined provider detail and throws, so `POST /api/chat`'s existing catch returns its `503` JSON error rather than an empty `200` SSE stream with a sources event. `src/routes/api/chat/+server.ts` needed no code changes — its existing error handling already satisfied this contract once the adapter stopped silently swallowing in-band failures.
**`CHAT-08B` fix (2026-07-07):** The prior `CHAT-07D` instruction ("treat retrieved excerpts as evidence to paraphrase, not prose to copy") was directionally correct but not concrete enough — the model still lifted long contiguous spans on strong-match sources. `src/lib/server/personality.ts` adds an explicit, numeric ceiling: "Never carry over more than about 8 consecutive words from any excerpt... a strong, highly relevant source is a reason to use the idea, not a license to quote its wording" — a quantitative rule a model can follow more reliably than a qualitative one, set below the audit's 12-word flag threshold with margin. `src/lib/server/chat.ts`'s per-chunk context label was also strengthened from `Evidence to paraphrase:` to `Evidence to paraphrase (rewrite in your own words, do not copy wording):` so the reminder sits immediately next to the excerpt text itself, not only in the system prompt. Retrieval, confidence gating, and low-confidence fallback logic were untouched. A post-fix `npm run chat:audit` sweep against the same 10-question set showed zero quote-copy flags: the previously-flagged `the-inspiration` (direct, high-tier) question dropped from a 31-word shared run to 7 words, and every other citable answer measured 0–8 words, all below the 12-word flag threshold.
