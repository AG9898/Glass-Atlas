...
- `POLISH-07F` shipped the first branded motion asset (§6): a wave-like, line-based 3D grid field.
  - `src/lib/components/WaveGridField.svelte` renders an SSR-safe static inline-SVG line-grid frame unconditionally, then — only in the browser and only when `shouldUseWaveGridMotion()` (`src/lib/motion/wave-grid.ts`) allows it — dynamically imports `three` inside `onMount` and layers a `<canvas>` on top driving a sparse `THREE.LineSegments` wireframe grid with a per-vertex sine-wave displacement.
  ...
  - Integrated into `src/routes/+page.svelte` as a bounded structural band between the hero and stats sections (full-bleed within its band, no card border/background panel), never overlapping hero copy/CTA/chat or stats text.
  - Colors are read live from the CSS custom properties (`--color-line-2`, `--color-line-3`, `--color-accent-500`, `--color-accent2-500`, `--color-bg`) via `getComputedStyle`, so light/dark parity is automatic; a hardcoded light-mode hex fallback covers non-browser/SSR contexts.
  - Reduced-motion users never trigger the `three` import — verified by inspecting the rendered DOM under `--force-prefers-reduced-motion` in headless Chrome, which showed no `<canvas>` element at all, only the static SVG frame.
