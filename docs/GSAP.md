...
- `POLISH-07J` added an expand/collapse control to the public chat panel (`src/lib/components/Chat.svelte`), per direct user request.
  ...
  - Follow-up fix (same feature): navigating while expanded (e.g. clicking a note/How It Works link inside the chat) used to strand the reparented section in `document.body` as an unclosable fixed overlay — SvelteKit's page teardown only removes the old page subtree, and the expanded panel was outside it. `forceCollapseNow()` (kill tweens, instant `restoreCollapsedState()`) now runs from both `beforeNavigate` and the component's `$effect` teardown, so the panel is back inside the subtree before Svelte removes the page. Verified via CDP in fixed-wrapper mode: after an in-chat SPA navigation, `.ga-chat`, backdrop, and placeholder are all gone, scroll works on the destination, and expand/collapse still works after navigating back.
