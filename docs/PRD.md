...
| 13 | Node View explains authored and inferred relationships | Open `/node-view`; toggle wiki-link and semantic edges independently, then expand the graph from a note and confirm that note is focused |

---

## Constraints

- **LLM grounding** — all chat responses must be derived from retrieved notes only. Hallucination outside retrieved context is not acceptable. Note *metadata* (the index of titles and takeaways used by the catalog and low-confidence steer paths) is navigation surface, not an answer source: it may be used to name and characterize notes, never to make a factual claim about the question asked.
