...
| `src/tests/auth-redirect.test.ts` | `src/hooks.server.ts`, `src/routes/signin/+page.server.ts` | Canonical www-to-apex redirect with path/query preservation and Railway/local exemptions; `buildSigninRedirectUrl` pure helper; sign-in load callbackUrl defaults and pass-through |
