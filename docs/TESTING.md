...
| `src/routes/api/chat/chat.test.ts` | `src/routes/api/chat/+server.ts` | Anonymous cookie issuance/reuse, malformed-cookie fallback, per-cookie quota isolation, DB-backed per-session quota enforcement, early 429 short-circuit (including limit+1), social-intent SSE path (no retrieval/LLM/citations), confidence-gate fallback (low-coverage returns SSE without LLM call, no citation recording), borderline confidence remains distinguishable while still using the LLM path, and high-confidence path (LLM called, context included in prompt) |
