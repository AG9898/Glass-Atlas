...
| OpenRouter chat adapter | `src/lib/server/ai/openrouter.ts` | Mock `fetch`; assert primary-model request shape, fallback-model retry on rate limits/provider failures/timeouts, no fallback for malformed successful streams, and stream parsing that ignores reasoning fields while preserving visible `delta.content` |
