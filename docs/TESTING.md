...
| DB query layer | `src/lib/server/db/notes.ts` | Unit tests with mocked Drizzle client (`vi.mock`), including centralized semantic-index freshness filtering for chat/related-note retrieval |
...
| Chat module | `src/lib/server/chat.ts` | Mock semantic and lexical retrieval; assert semantic query alias expansion, parallel call dispatch, hybrid candidate fusion ordering, deduplication, 5-note cap, natural-language lexical token matching, lexical-only note formatting, confidence tiers, stale/pending index exclusion, low-confidence source/link suppression, quote-copy guard helpers, and compact prompt assembly |
| API route — chat | `src/routes/api/chat/+server.ts` | Import handler directly, call with mock `Request`; assert rate limiting, streaming response shape, low-confidence fallback (no LLM call and no irrelevant source/related links), borderline limited-coverage LLM prompt, high-confidence path (context included in normal LLM prompt), and server-provided source metadata when retrieval yields citable notes |
| OpenRouter chat adapter | `src/lib/server/ai/openrouter.ts` | Mock `fetch`; assert primary-model request shape, fallback-model retry on rate limits/provider failures/timeouts, fallback retry on an in-band SSE `error` event or a stream that completes with zero visible content, fallback retry when the primary stream stalls past the inactivity timeout, graceful `[DONE]`-and-close (no retry) when an in-band error arrives after content has already streamed, logged provider detail plus rejection when both models fail, and stream parsing that ignores reasoning fields while preserving visible `delta.content` |
...
| Semantic index maintenance script | `scripts/semantic-index.js` | Mock the maintenance modules; assert audit issue detection, required env validation, and refresh routing through `reindexNoteAfterSave()` |
...
| Homepage streamed data | `src/routes/+page.server.ts` (`landing` promise), `src/routes/+page.svelte` | No component-level browser automation in v1; verify via the CDP technique below that the raw SSR response contains the `.stat-skeleton` pending markup and omits `data-motion="stat"` (proving `{:then}` isn't in the synchronous output), then that a follow-up `Runtime.evaluate` after load shows real stat values, zero leftover skeleton nodes, and normal splash fade-out |
...
| `src/routes/api/chat/chat.test.ts` | `src/routes/api/chat/+server.ts` | Anonymous cookie issuance/reuse, malformed-cookie fallback, per-cookie quota isolation, DB-backed per-session quota enforcement, early 429 short-circuit (including limit+1), social-intent SSE path (no retrieval/LLM/citations, templated non-factual replies), confidence-gate fallback (low-coverage returns SSE without LLM call, no citation recording, and does not pass low-confidence candidates into fallback copy), borderline limited-coverage prompt shape, high-confidence path (LLM called, context included in normal prompt), and the source-metadata contract (`buildChatSources` called with `citedNotes` only on the citable LLM path, a trailing `sources` SSE event appended when sources exist, the event omitted when `buildChatSources` returns none, and `buildChatSources` never called on the low-confidence fallback or social-intent lanes), and the catalog lane (retrieval + confidence gate bypassed entirely, instruction/index block passed to the LLM, catalog notes attached as sources, deterministic answer served instead of a 503 when the LLM fails, fall-through to retrieval when nothing is published, no citation recording, and rate limiting enforced before the metadata query) |
...
| `src/tests/auth-redirect.test.ts` | `src/hooks.server.ts`, `src/routes/signin/+page.server.ts` | Canonical www-to-apex redirect with path/query preservation, Railway/local exemptions, protocol-relative open-redirect containment, and security headers on the 308; `buildSigninRedirectUrl` pure helper; sign-in load callbackUrl defaults and pass-through |
...
| `src/lib/server/chat.test.ts` | `src/lib/server/chat.ts` | Semantic query alias expansion, parallel semantic + lexical retrieval, candidate fusion ordering (semantic-first then lexical fill), deduplication of overlapping slugs, per-note chunk grouping (cap 2), fused 5-note cap, lexical-only note formatting (title + takeaway), synthesis-oriented context labels, section heading inclusion/omission, slug/title inclusion, ranked citation slug order, empty-result handling, confidence tier boundary cases, `INSUFFICIENT_COVERAGE_RESPONSE` first-person voice contract, `buildFallbackResponse` (no-coverage steer with no related-note footer), `citedNotes` population from semantic chunks and lexical notes (including the derived `snippet` field and its title fallback when a lexical note has no takeaway), `buildChatSources` (safe-slug passthrough, unsafe-slug filtering, empty-snippet filtering, empty-input handling), `detectCatalogIntent` (recency/count/list classification plus the negative cases that must stay on the retrieval path, such as "what have you written about agent infrastructure?"), `buildCatalogContext` (ordering, dates, takeaways, empty-corpus null, per-intent instruction, body exclusion, enumeration cap, intent-scoped source limits, missing date/takeaway handling), and `buildCatalogFallbackResponse` (names the newest note, states the total, singular/plural agreement) |
...
| `src/tests/semantic-index-script.test.ts` | `scripts/semantic-index.js` | Published-note semantic-index maintenance coverage for CLI mode parsing, required env validation, stale/missing chunk issue reporting, and refresh calls through `reindexNoteAfterSave()` |
...
- The low-confidence index steer (RESOLVED-28) splits cleanly into testable and non-testable properties. Unit-testable and required: the index block is passed on the low tier only and is absent from high/borderline prompts; the block contains no note bodies or chunk prose; no `sources` SSE event is emitted on this path; the deterministic index-derived response is served when the LLM call fails, returns empty, or reports an in-band provider error. Not unit-testable: whether the model actually refrained from answering from a takeaway, and whether it correctly stopped instead of reciting the corpus on an off-topic question — both are judgment properties of generated prose. Cover those in the `npm run chat:audit` sweep with at least one genuinely off-corpus question (an unrelated-domain prompt) and one near-miss question (a topic adjacent to a real note that retrieval fails to surface), reviewing the output manually for a factual claim that the index could not support.
- Catalog-lane intent detection is the highest-value test surface in that lane: its risk is not a missed match (which simply falls through to normal retrieval) but a false positive answering a content question with a table of contents. Any new phrasing added to `CATALOG_INTENT_PATTERNS` should arrive with a paired negative case in the `leaves %j on the retrieval path` table in `src/lib/server/chat.test.ts`.
...
- Add deterministic unit coverage around prompt text and route message assembly so the system prompt requires conversational synthesis, permits fuller answers when evidence supports them, and warns against verbatim excerpt copying unless the user asks for a quote.
- Add a quote-copy regression helper or fixture that fails when generated/fixture answers copy more than a short contiguous run from retrieved excerpts without quote intent. Keep this deterministic in CI; real OpenRouter audit sweeps belong in manual or script-driven verification, not unit tests.
- For model fallback, use mocked SSE streams that include both `reasoning` and `content` deltas. Assert the client-visible parser ignores reasoning and still emits visible answer text.
- After chat-quality implementation work, run a live audit sweep with local `.env`/`.env.local` or Railway-provided env vars against representative direct, adjacent, and unrelated questions. Record model availability, top retrieved chunks/distances, answer fluidity, quote-copy diagnostics, and low-confidence fallback behavior.
- The sweep is scripted: `npm run chat:audit` (`scripts/chat-audit.js`) runs a fixed direct/adjacent/unrelated question set and emits a JSON report covering retrieval diagnostics (semantic query expansion, top chunk cosine distances, lexical matches, confidence tier, cited/source slugs), the live `POST /api/chat` streamed answer + trailing `sources` event (fresh anonymous session per request, so it never trips the per-session quota), quote-copy diagnostics (longest contiguous shared word run between answer and retrieved excerpts; flagged at ≥ 12 words), and direct OpenRouter probes of the configured primary/fallback models that distinguish HTTP-level failures from in-band SSE `error` events arriving with HTTP 200. It requires `DATABASE_URL` + `OPENROUTER_API_KEY` and a running dev server at `--base-url` (default `http://localhost:5173`) unless `--skip-live` is passed. Its live requests consume real free-model quota and bump note citation counters, so it is manual tooling — never wire it into CI.
- `CHAT-08B` (2026-07-07) found a qualitative paraphrase instruction ("treat excerpts as evidence to paraphrase") was not concrete enough on its own — a 31-word verbatim run still slipped through on a direct, high-tier question. Adding a concrete numeric ceiling ("never carry over more than about 8 consecutive words from any excerpt") to `personality.ts`, plus repeating the reminder inline in `chat.ts`'s per-chunk context label, brought a full 10-question re-audit to zero quote-copy flags (max observed run: 8 words, well under the 12-word flag threshold). When tuning prompt-following behavior for a small free-tier model, prefer a concrete, checkable numeric rule over a qualitative one where possible — it is easier for the model to satisfy and easier to verify against the audit's own threshold.
