...
| Admin embedding refresh behavior | Admin note create/edit server actions and embedding helpers | Mock embedding calls; assert successful saves replace vectors, failed refreshes preserve previous embeddings/chunks, and stale-index metadata is recorded for admin display |
