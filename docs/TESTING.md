...
| Wave-grid 3D motion asset | `src/lib/motion/wave-grid.ts`, `src/lib/components/WaveGridField.svelte` | Pure unit tests for reduced-motion gating and CSS-custom-property palette resolution (including its non-browser fallback); the actual Three.js scene/canvas has no component-rendering test and was instead verified with a one-off headless-Chrome browser smoke check (see below) |
