...
- **Reworked (`POLISH-07H`):** the wave-grid band previously shipped here in `POLISH-07F` (see §7.3) has been removed. The asset is now the shared loader/spinner primitive used for the route-transition overlay, the initial-load splash, and Chat's inline "Searching notes…" state, rather than a homepage decoration. Hero flows directly into the stats section, separated by `.hero`'s own bottom rule (`.motion-rule--bottom`).
...
- **Shipped (`POLISH-07F`):** implemented as a real-time Three.js scene, factored into `src/lib/motion/wave-grid-engine.ts` (scene/layer construction) and `src/lib/components/WaveGridLoader.svelte` (SSR-safe static CSS grid fallback, reduced-motion permanent state; the animated `THREE.LineSegments` wireframe overlay only mounts in the browser when motion is allowed) — see `docs/GSAP.md` §9 for the full implementation note. Uses only neutral line tokens plus a small minority of sage/warm-taupe accent lines.
- **Reworked (`POLISH-07H`):** rather than sitting as a decorative band on the landing page, this asset is now the shared loader/spinner primitive across the app. `WaveGridLoader` takes a `variant: 'overlay' | 'compact'` prop (full-viewport vs. small inline density) and is mounted in three places: the route-transition overlay, the initial-load boot splash, and inline next to Chat's "Searching notes…" state. It no longer appears on the landing page.
...
- **Shipped (`POLISH-07E`):** public route changes play a fast branded wipe overlay (`PageTransitionOverlay.svelte`, mounted once in `+layout.svelte`, outside the smooth-scroll wrapper), masked in and back out with a CSS `clip-path: inset(...)` tween — the mask boundary itself is the "sharp rectangular mask" called for above. The overlay stays `pointer-events: none` for its whole lifecycle so it never traps interaction, and reduced-motion users get plain instant navigation because the cover/reveal tweens never run (gated on `canUseSpatialMotion`, with a CSS `prefers-reduced-motion` rule as a backstop). See [GSAP.md](GSAP.md) section 9 for implementation detail.
- **Reworked (`POLISH-07H`):** the overlay's background is now the wave-grid loader (`<WaveGridLoader variant="overlay" />`), mounted only while the overlay covers the screen and unmounted once the reveal tween completes — the plain repeating-vertical-rule background is retired in favor of the same branded 3D grid used elsewhere. A matching `InitialLoadScreen.svelte` (public routes only, boot-only, fades out once fonts are ready) uses the same overlay-variant loader as a first-paint splash.
